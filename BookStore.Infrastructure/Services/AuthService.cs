    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<(ServiceResult<LoginUserResponseDto> Result, string? RefreshToken, DateTime? Expires)> LoginAsync(LoginUserDto loginUserDto)
        {
            try
            {
                ...
                {
                    var AccessToken = tokenService.GenerateAccessToken(user.Id, user.Email!, user.EmailConfirmed);
                ...
                var newAccessToken = tokenService.GenerateAccessToken(user.Id, user.Email!, user.EmailConfirmed);
        ...
        public async Task<(ServiceResult<RegisterResponseDto> Result, string? RefreshToken, DateTime? Expires)> RegisterAsync(RegisterUserDto registerUserDto)
        {
            try
            {
                ...
                var accessToken = tokenService.GenerateAccessToken(newUser.Id, newUser.Email!, newUser.EmailConfirmed);
